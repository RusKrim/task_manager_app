...
import {availableTags} from '../../bus/tasks/availableTags';

export const Tag = ({ selected, type, cb }) => {
  let chosenTagType;
  ...
  const tagCn = cx('tag', chosenTagType, {selected:selected});
  const tagClick = () => {
  ...
  return (
  <span onClick={tagClick} className={tagCn}>{type}</span>
  )
};
